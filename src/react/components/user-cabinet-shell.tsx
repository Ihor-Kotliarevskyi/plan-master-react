...
export function UserCabinetShell() {
  ...
  useEffect(() => {
    ...
    const handleBackdropClick = (event: MouseEvent) => {
      if (event.target === modalRoot) closeUserCabinetModal();
    };
  ...
  return (
    <div className="modal um-modal" onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true">
      <div className="modal-header modal-header-lg">
        <h3>Profile</h3>
        <button className="btn btn-sm" onClick={closeUserCabinetModal} type="button">x</button>
      </div>
      <div className="user-modal-body react-user-cabinet" id="user-modal-body">
          ...
          <div className="um-col">
            <div className="um-user-card">
              ...
              <div className="um-user-info">
                <label htmlFor="react-um-name" className="user-inline-label">Р†Рј'СЏ</label>
                <input
                  id="react-um-name"
                  className="user-inline-input"
                  value={profileName}
                  placeholder="Р’РІРµРґС–С‚СЊ С–Рј'СЏ"
                  onChange={(event) => setProfileName(event.target.value)}
                />
                <span className="user-email-hint">{snapshot.profile.email || "email РЅРµ РІРєР°Р·Р°РЅРѕ"}</span>
      ...
      <div className="m-btns m-btns-sep">
        <button className="btn" onClick={closeUserCabinetModal} type="button">РЎРєР°СЃСѓРІР°С‚Рё</button>
        <button className="btn btn-acc" onClick={handleProfileSave} disabled={pending === "profile" || !snapshot.loggedIn && pending === "auth"} type="button">
          {pending === "profile" ? "Р—Р±РµСЂРµР¶РµРЅРЅСЏ..." : "Р—Р±РµСЂРµРіС‚Рё"}
        </button>
      </div>
